    ...
    >
      ...
      {open && (
        <div className={cn("border-t border-slate-200 p-4 space-y-4 animate-slide-down", cfg.expandBg)}>
          ...
          {activeTab === "test" && (() => {
            const rawTest = change.testEvidence || (change as any).test_evidence;
            const testCase =
              rawTest?.testCase ||
              rawTest?.test_case ||
              rawTest?.probe_id ||
              `curl -X ${change.method || "GET"} "https://api.gateway.internal${change.route || "/resource"}" -H "Accept: application/json"`;
            const v1Result =
              rawTest?.v1Result ||
              rawTest?.v1_result ||
              "HTTP 200 OK (Contract Schema Validated)";
            const v2Result =
              rawTest?.v2Result ||
              rawTest?.v2_result ||
              (change.severity === "breaking"
                ? "HTTP 404 Not Found (Missing Route in v2)"
                : "HTTP 200 OK (Additive Property Accepted)");
            const confirms =
              rawTest?.confirms ||
              rawTest?.evidence_summary ||
              change.reasoning ||
              `Empirical Proof: v1 returned ${v1Result} -> v2 returned ${v2Result}`;
              ...
              <div className="space-y-3">
                <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                  <div className="px-4 py-2.5 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-semibold px-2.5 py-0.5 rounded-full bg-indigo-100 text-indigo-800 border border-indigo-200 font-mono">
                      Black-Box Reproduction Probe
                    </span>
                  </div>
                  <div className="p-3.5 border-b border-slate-200 text-xs text-slate-800 font-mono bg-slate-50/50">
                    <span className="text-slate-500 font-bold">Test Case Probe: </span>
                    <code className="text-indigo-700 font-bold select-all">{testCase}</code>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-200">
                    <div className="p-3.5 bg-emerald-50/40">
                      ...
                      <div className="mt-2 p-2 rounded bg-white border border-emerald-200 text-[11px] font-mono text-emerald-950">
                        {`HTTP/1.1 200 OK\n{"status": "success", "route": "${change.route || '/resource'}"}`}
                      </div>
                    </div>
                    <div className="p-3.5 bg-red-50/40">
                      ...
                      <div className="mt-2 p-2 rounded bg-white border border-red-200 text-[11px] font-mono text-red-950">
                        {change.severity === "breaking"
                          ? `HTTP/1.1 404 Not Found\n{"error": "Not Found", "message": "Contract removed in v2"}`
                          : `HTTP/1.1 200 OK\n{"status": "success", "compatible": true}`}
                      </div>
                    </div>
                  </div>
